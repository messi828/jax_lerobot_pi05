...
class OpenCVCamera(Camera):
    ...
    def __init__(self, config: OpenCVCameraConfig, *, name: str | None = None):
        """
        Initializes the OpenCVCamera instance.

        Args:
            config: The configuration settings for the camera.
            name: Optional display name (e.g. from robot config key like "fixed" or "handeye")
                  used in logs and errors instead of the device path.
        """
        super().__init__(config)

        self.config = config
        self.name = name  # user-defined name from config key, for logs
    ...
    def __str__(self) -> str:
        if self.name is not None:
            return f"{self.__class__.__name__}({self.name!r}, {self.index_or_path})"
    ...
    def _validate_fps(self) -> None:
        ...
        # Some Linux/V4L2 drivers report set() failure even when the value is applied.
        if not math.isclose(self.fps, actual_fps, rel_tol=1e-3):
            logger.warning(
                "%s requested fps=%s but camera reports %s; using actual fps.",
                self, self.fps, actual_fps,
            )
            self.fps = actual_fps
        elif not success:
            logger.warning(
                f"{self} reported fps set failure but applied target value ({self.fps=}, {actual_fps=}). "
                "Continuing."
            )
    ...
    def _validate_width_and_height(self) -> None:
        ...
        if self.capture_width != actual_width:
            logger.warning(
                "%s requested capture_width=%s but camera reports %s; using actual resolution.",
                self, self.capture_width, actual_width,
            )
            self.capture_width = actual_width
        elif not width_success:
            logger.warning(
                f"{self} reported width set failure but applied target value "
                f"({self.capture_width=}, {actual_width=}). Continuing."
            )

        actual_height = int(round(self.videocapture.get(cv2.CAP_PROP_FRAME_HEIGHT)))
        if self.capture_height != actual_height:
            logger.warning(
                "%s requested capture_height=%s but camera reports %s; using actual resolution.",
                self, self.capture_height, actual_height,
            )
            self.capture_height = actual_height
        elif not height_success:
            logger.warning(
                f"{self} reported height set failure but applied target value "
                f"({self.capture_height=}, {actual_height=}). Continuing."
            )
