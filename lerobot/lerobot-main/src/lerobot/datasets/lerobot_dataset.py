        ...
        try:
            ...
            if root_was_explicit:
                if not (self.root / INFO_PATH).exists():
                    raise FileNotFoundError(
                        f"Dataset root is set to {self.root} but {INFO_PATH} was not found there. "
                        "Ensure the directory contains the full LeRobot dataset (with meta/info.json)."
                    ) from None
                # root 已设置且 meta 存在但仍失败：不访问网络，直接抛出原始错误
                raise e
            # 默认 cache 下已有 meta/info.json 时，用本地版本重试，避免无网络时 get_safe_version 失败
            if (self.root / INFO_PATH).exists():
                info = load_json(self.root / INFO_PATH)
                self.revision = info.get("codebase_version", self.revision)
                self.load_metadata()
                return
