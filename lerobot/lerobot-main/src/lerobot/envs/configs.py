...
class InverseKinematicsConfig:
    ...
    end_effector_bounds: 末端执行器位置边界，单位米，基座坐标系 [x,y,z]。
      格式 {"min": [x_min, y_min, z_min], "max": [x_max, y_max, z_max]}。
      z 越小表示越低；若希望末端能更往下，可把 min 的 z 改小（如 0.025 -> 0.01）。
    end_effector_position_offset: 可选 [dx, dy, dz]（米），在裁剪前加到目标位置上，
      用于补偿 URDF 与实机偏差。例如 [0, 0, -0.02] 表示整体命令往下 2cm。
    """
    ...
    end_effector_position_offset: list[float] | None = None
