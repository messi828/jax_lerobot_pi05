...
class RobotKinematics:
    ...
    def __init__(
        ...
        # Placo expects a robot directory (containing `robot.urdf`), while many
        # user configs provide a direct `.urdf` file path.
        robot_source = Path(urdf_path).expanduser()
        if not robot_source.is_absolute():
            robot_source = (Path.cwd() / robot_source).resolve()
        if robot_source.suffix.lower() == ".urdf":
            if not robot_source.exists():
                # Try paths relative to common project roots (e.g. lerobot-main)
                for root in [Path.cwd(), Path(__file__).resolve().parents[3]]:
                    candidate = (root / urdf_path).resolve()
                    if candidate.is_file():
                        robot_source = candidate
                        break
            if robot_source.is_file():
                ...
                if not robot_urdf.exists():
                    try:
                        robot_urdf.symlink_to(robot_source.name)
                    except OSError:
                        shutil.copyfile(robot_source, robot_urdf)
                placo_robot_path = str(robot_dir)
            else:
                placo_robot_path = str(robot_source)
        else:
            placo_robot_path = str(robot_source)

        self.robot = placo.RobotWrapper(placo_robot_path)
