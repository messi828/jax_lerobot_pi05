    ...
    **kwargs: Unpack[ProcessorConfigKwargs],
    ...
    if pretrained_path:
        ...
        if isinstance(policy_cfg, GrootConfig):
            ...
            # Merge with any user-provided overrides (e.g. device_processor device=cpu).
...
def _maybe_fix_pi05_adapter_layernorm_keys(
    policy: torch.nn.Module,
    peft_pretrained_path: str | Path,
    peft_config: Any,
) -> str | Path:
    """若 adapter 为旧版 gemma_expert layernorm（.weight）而 base 为 .dense.weight/.dense.bias，用 base 补全 adapter 后返回临时目录，否则返回原路径。"""
    import os

    from safetensors.torch import load_file, save_file

    path = Path(peft_pretrained_path)
    adapter_file = path / "adapter_model.safetensors"
    if not adapter_file.exists():
        return peft_pretrained_path

    adapter_sd = load_file(str(adapter_file))
    # adapter 里是否有旧格式（无 .dense）的 layernorm 或 gemma_expert.model.norm
    has_old_layernorm = any(
        re.search(r"\.(input_layernorm|post_attention_layernorm)\.weight$", k)
        and ".dense." not in k
        for k in adapter_sd.keys()
    )
    has_old_norm = any(
        "gemma_expert.model.norm.weight" in k and "norm.dense." not in k
        for k in adapter_sd.keys()
    )
    if not has_old_layernorm and not has_old_norm:
        return peft_pretrained_path
    ...
    for key, value in base_sd.items():
        if not key.startswith("model."):
            continue
        # model.paligemma_with_expert.gemma_expert.model.layers.N.input_layernorm.dense.(weight|bias)
        if re.search(
            r"model\.paligemma_with_expert\.gemma_expert\.model\.layers\.\d+\.(input_layernorm|post_attention_layernorm)\.dense\.(weight|bias)$",
            key,
        ):
            adapter_key = prefix + key
            if adapter_key not in adapter_sd:
                adapter_sd[adapter_key] = value.clone()
                added += 1
        # model.paligemma_with_expert.gemma_expert.model.norm.dense.(weight|bias)
        elif re.search(
            r"model\.paligemma_with_expert\.gemma_expert\.model\.norm\.dense\.(weight|bias)$",
            key,
        ):
            adapter_key = prefix + key
            if adapter_key not in adapter_sd:
    ...
    logging.info(
        "Patching PI05 adapter: added %s base layernorm/norm.dense keys for compatibility.",
        added,
    )
    tmpdir = tempfile.mkdtemp(prefix="lerobot_peft_adapter_")
    try:
        for f in path.iterdir():
            if f.is_file():
                shutil.copy2(f, Path(tmpdir) / f.name)
        save_file(adapter_sd, os.path.join(tmpdir, "adapter_model.safetensors"))
        return tmpdir
    except Exception:
...
def make_policy(
    ...
    if cfg.pretrained_path and not cfg.use_peft:
        ...
        policy = policy_cls.from_pretrained(**kwargs)
        # 若 adapter 含旧版 layernorm key（.weight）而 base 为 .dense.weight/.dense.bias，用 base 补全 adapter 后从临时目录加载
        load_path = _maybe_fix_pi05_adapter_layernorm_keys(
            policy, peft_pretrained_path, peft_config
        )
        policy = PeftModel.from_pretrained(policy, load_path, config=peft_config)
