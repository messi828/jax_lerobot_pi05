...
class EagleBackbone(nn.Module):
    ...
    def set_trainable_parameters(self, tune_llm: bool, tune_visual: bool):
        ...
        use_llm_lora = getattr(self.eagle_model, "use_llm_lora", False)
        if use_llm_lora:
            # LoRA is applied: do not touch language_model (PEFT keeps base frozen, LoRA trainable).
            # Only freeze vision/mlp1 when not tuning visual.
            if not tune_visual:
                self.eagle_model.vision_model.requires_grad_(False)
                self.eagle_model.mlp1.requires_grad_(False)
            self.eagle_linear.requires_grad_(True)
        else:
...
class GR00TN15(PreTrainedModel):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: str, **kwargs):
        ...
        print(f"Tune backbone LLM: {tune_llm}" + (f" (LoRA r={lora_rank})" if tune_llm and lora_rank else ""))
        ...
        if tune_llm and lora_rank > 0 and hasattr(pretrained_model.backbone, "eagle_model"):
            if not getattr(pretrained_model.backbone.eagle_model, "use_llm_lora", False):
                pretrained_model.backbone.eagle_model.wrap_llm_lora(
                    r=lora_rank, lora_alpha=lora_alpha, lora_dropout=lora_dropout
                )
                print(f"[GROOT] LLM wrapped with LoRA rank={lora_rank} (saves VRAM vs full fine-tune).")
