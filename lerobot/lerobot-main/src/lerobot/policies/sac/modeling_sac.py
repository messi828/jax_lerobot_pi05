...
class PretrainedImageEncoder(nn.Module):
    ...
    def _load_pretrained_vision_encoder(self, config: SACConfig):
        ...
        name_or_path = config.vision_encoder_name
        # 本地路径时强制离线加载，避免无网络时反复请求 HuggingFace
        local_files_only = isinstance(name_or_path, str) and (
            name_or_path.startswith("/") or name_or_path.startswith(".")
        )
        self.image_enc_layers = AutoModel.from_pretrained(
            name_or_path, trust_remote_code=True, local_files_only=local_files_only
        )
