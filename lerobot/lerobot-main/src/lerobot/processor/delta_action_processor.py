...
class MapDeltaActionToRobotActionStep(RobotActionProcessorStep):
    ...
    def action(self, action: RobotAction) -> RobotAction:
        ...
        # When action is from teleop (e.g. leader-follow), always enable so follower follows every step
        if self.transition.get(TransitionKey.COMPLEMENTARY_DATA, {}).get("action_from_teleop", False):
            enabled = True
