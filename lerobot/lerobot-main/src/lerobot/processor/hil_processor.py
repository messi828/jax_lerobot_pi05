...
class AddTeleopActionAsComplimentaryDataStep(ComplementaryDataProcessorStep):
    ...
    def complementary_data(self, complementary_data: dict) -> dict:
        ...
        # When actor provided teleop for this step (intervention), use it so delta and observation are from same moment → 跟准
        if "actor_teleop_action" in complementary_data:
            new_complementary_data[TELEOP_ACTION_KEY] = complementary_data["actor_teleop_action"]
        else:
...
class AddTeleopEventsAsInfoStep(InfoProcessorStep):
    ...
    def info(self, info: dict) -> dict:
        ...
        If the transition's complementary_data already has "actor_teleop_events" (set by the
        actor after get_teleop_events()), use that so s/q keys are not consumed twice and
        episode end/success take effect immediately.
        """
        new_info = dict(info)
        # Prefer actor-provided events so s/q read once in actor apply in same step
        comp = {}
        if hasattr(self, "_current_transition"):
            comp = getattr(self, "_current_transition", {}).get(TransitionKey.COMPLEMENTARY_DATA) or {}
        if isinstance(comp, dict) and "actor_teleop_events" in comp:
            teleop_events = comp["actor_teleop_events"]
        else:
...
class InterventionActionProcessorStep(ProcessorStep):
    ...
    def __call__(self, transition: EnvTransition) -> EnvTransition:
        ...
        # Handle episode termination (only s/q/r end the episode, not intervention alone)
        new_transition[TransitionKey.DONE] = bool(terminate_episode) or (
            self.terminate_on_success and success
        )
        # Reward=1 for success (s key) or for intervention steps (human control segment)
        new_transition[TransitionKey.REWARD] = float(success or is_intervention)
        ...
        # Update complementary data with teleop action and flag for downstream (e.g. gripper position mode)
        complementary_data = new_transition.get(TransitionKey.COMPLEMENTARY_DATA, {})
        complementary_data[TELEOP_ACTION_KEY] = new_transition.get(TransitionKey.ACTION)
        complementary_data["action_from_teleop"] = bool(is_intervention and teleop_action is not None)
