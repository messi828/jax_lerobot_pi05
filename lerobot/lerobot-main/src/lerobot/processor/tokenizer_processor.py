    ...
    def __post_init__(self):
        ...
        if self.tokenizer is not None:
            ...
            try:
                ...
                if "Unrecognized model" in str(e) and (
                    "pi05" in (self.tokenizer_name or "").lower()
                    or "paligemma" in (self.tokenizer_name or "").lower()
                    or "lerobot" in (self.tokenizer_name or "").lower()
                ):
                    logging.warning(
                        "Tokenizer path %s is not a valid HuggingFace tokenizer config; "
                        "using PaliGemma tokenizer for PI05 compatibility (offline cache).",
                        self.tokenizer_name,
                    )
                    try:
                    ...
                    except (OSError, ValueError) as offline_err:
                        raise RuntimeError(
                            "PaliGemma tokenizer for PI05 is not in the local HuggingFace cache "
                            "and network to huggingface.co is unavailable. Pre-download the tokenizer when online: "
                            "python -c \"from transformers import AutoTokenizer; AutoTokenizer.from_pretrained('google/paligemma-3b-pt-224')\" "
                            "or: huggingface-cli download google/paligemma-3b-pt-224"
                        ) from offline_err
                else:
                    raise
        else:
