...
from lerobot.teleoperators import gamepad, keyboard, so_leader, so_leader_spacebar_follow  # noqa: F401
...
def act_with_policy(
    ...
    # Initial reset: leader and follower reset in parallel
    reset_cfg = getattr(cfg.env.processor, "reset", None)
    reset_pose = getattr(reset_cfg, "fixed_reset_joint_positions", None) if reset_cfg else None
    if hasattr(teleop_device, "notify_reset") and reset_pose is not None:
        sig = inspect.signature(teleop_device.notify_reset)
        notify_kw = {"duration_sec": 6.0} if "duration_sec" in sig.parameters else {}
        reset_result = []

        def _initial_env_reset():
            reset_result.append(online_env.reset())

        env_reset_thread = threading.Thread(target=_initial_env_reset)
        env_reset_thread.start()
        teleop_device.notify_reset(reset_pose, **notify_kw)
        env_reset_thread.join()
        obs, info = reset_result[0] if reset_result else online_env.reset()
        if not notify_kw:
            precise_sleep(2.0)
    else:
    ...
    for interaction_step in range(cfg.policy.online_steps):
        ...
        # Same as record mode: get teleop state first (drains keys, updates leader arm)
        if hasattr(teleop_device, "get_action"):
            teleop_action_dict = teleop_device.get_action()
        else:
            teleop_action_dict = None
        ...
        if hasattr(teleop_device, "get_teleop_events"):
            ...
            # 把 s/q 等事件传入 pipeline，否则 pipeline 里再次 get_teleop_events() 时队列已空，按 s 不能马上结束给奖励
            if transition.get(TransitionKey.COMPLEMENTARY_DATA) is None:
                transition[TransitionKey.COMPLEMENTARY_DATA] = {}
            transition[TransitionKey.COMPLEMENTARY_DATA]["actor_teleop_events"] = teleop_events
            if teleop_events.get(TeleopEvents.SUCCESS, False):
                logging.info("[ACTOR] s pressed → ending episode and giving reward=1 for trajectory")
            if teleop_events.get(TeleopEvents.TERMINATE_EPISODE, False) and not teleop_events.get(TeleopEvents.SUCCESS, False):
                logging.info("[ACTOR] q/r pressed → ending episode")

        # On first step of intervention: reset action processor so no policy state is carried over
        if is_intervention and not was_intervention:
            action_processor.reset()
        was_intervention = is_intervention

        # Intervention: use teleop from same moment as observation so target = leader pose → 跟准
        raw_observation_override = None
        comp = transition.get(TransitionKey.COMPLEMENTARY_DATA)
        if comp is not None and "actor_teleop_action" in comp and not is_intervention:
            comp.pop("actor_teleop_action", None)

        # 介入时：采集模式 = 主臂关节角直接驱动从臂；给模型学习的 action 仍为从臂侧 delta
        robot_action_override = None
        if is_intervention and teleop_action_dict is not None:
            # Observation right after get_action() so delta and obs same moment
            if hasattr(online_env, "get_raw_joint_positions"):
                raw_observation_override = online_env.get_raw_joint_positions()
            if transition.get(TransitionKey.COMPLEMENTARY_DATA) is None:
                transition[TransitionKey.COMPLEMENTARY_DATA] = {}
            transition[TransitionKey.COMPLEMENTARY_DATA]["actor_teleop_action"] = teleop_action_dict
            device = (
                list(observation.values())[0].device
                if observation and len(observation) > 0
                else torch.device("cpu")
            )
            # 采集模式：用主臂关节角直接驱动从臂（与 lerobot_record 一致）
            if hasattr(teleop_device, "get_leader_joint_positions"):
                leader_joints = teleop_device.get_leader_joint_positions()
                if leader_joints is not None:
                    robot_action_override = torch.tensor(
                        leader_joints, dtype=torch.float32, device=device
                    )
            action_dim = getattr(online_env.action_space, "shape", (4,))[0]
            action_list = [
                float(teleop_action_dict.get("delta_x", 0.0)),
                float(teleop_action_dict.get("delta_y", 0.0)),
                float(teleop_action_dict.get("delta_z", 0.0)),
            ]
            if action_dim > 3:
                action_list.append(float(teleop_action_dict.get("gripper", 1.0)))
        ...
        # 串口偶发 "There is no status packet!"，重试几步避免复位后立刻读总线导致崩溃
        last_err = None
        for attempt in range(3):
            try:
                new_transition = step_env_and_process_transition(
                    ...
                    raw_observation_override=raw_observation_override,
                    robot_action_override=robot_action_override,
                )
                break
        ...
        if done or truncated:
            ...
            # 新回合应由策略控制，不再默认人类接管
            if hasattr(teleop_device, "reset_for_new_episode"):
                teleop_device.reset_for_new_episode()

            update_policy_parameters(policy=policy, parameters_queue=parameters_queue, device=device)

            if len(list_transition_to_send_to_learner) > 0:
                # When user pressed 's' (success), backfill reward=1 for the whole episode trajectory
                success = intervention_info.get(TeleopEvents.SUCCESS, False)
                if success:
                    for t in list_transition_to_send_to_learner:
                        t["reward"] = 1.0
            ...
            # Leader and follower reset in parallel: leader resets itself on round-end signal
            reset_cfg = getattr(cfg.env.processor, "reset", None)
            reset_pose = getattr(reset_cfg, "fixed_reset_joint_positions", None) if reset_cfg else None
            if hasattr(teleop_device, "notify_reset") and reset_pose is not None:
                sig = inspect.signature(teleop_device.notify_reset)
                notify_kw = {"duration_sec": 6.0} if "duration_sec" in sig.parameters else {}
                # Run env.reset() and leader reset in parallel so they move together
                reset_result = []

                def _env_reset():
                    reset_result.append(online_env.reset())

                env_reset_thread = threading.Thread(target=_env_reset)
                env_reset_thread.start()
                teleop_device.notify_reset(reset_pose, **notify_kw)
                env_reset_thread.join()
                if reset_result:
                    obs, info = reset_result[0]
                else:
                    obs, info = online_env.reset()
                if not notify_kw:
                    precise_sleep(2.0)
                # 复位后恢复时间：避免串口刚忙完立刻读，并让机械臂稳定后再开始下一轮推理
                time.sleep(2.0)
            else:
                obs, info = online_env.reset()
            env_processor.reset()
            action_processor.reset()
            was_intervention = False  # 新回合从策略控制开始，介入状态已由 reset_for_new_episode 清除
