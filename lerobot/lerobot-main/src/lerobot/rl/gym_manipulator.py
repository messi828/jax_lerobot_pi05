...
from lerobot.teleoperators import (
    ...
    so_leader,  # noqa: F401
    so_leader_spacebar_follow,  # noqa: F401
)
...
def reset_follower_position(
    robot_arm: Robot, target_position: np.ndarray, duration_sec: float = 6.0
) -> None:
    """Reset robot arm to target position using smooth trajectory.

    Args:
        robot_arm: Robot to move.
        target_position: Target joint positions.
        duration_sec: Total time for the motion (longer = slower, safer for the arm). Default 6s.
    """
    ...
    trajectory = torch.from_numpy(
        np.linspace(current_position, target_position, num_steps)
    )
...
class RobotEnv(gym.Env):
    ...
    def reset(
        ...
        if self.reset_pose is not None:
            ...
            duration = min(self.reset_time_s * 0.8, 12.0)
            reset_follower_position(self.robot, np.array(self.reset_pose), duration_sec=duration)
...
def make_robot_env(cfg: HILSerlRobotEnvConfig) -> tuple[gym.Env, Any]:
    ...
    if hasattr(teleop_device, "set_robot"):
    ...
    if cfg.processor.inverse_kinematics is not None and kinematics_solver is not None:
        ...
        inverse_kinematics_steps = [
            ...
            EEBoundsAndSafety(
                ...
                position_offset=(
                    np.array(cfg.processor.inverse_kinematics.end_effector_position_offset, dtype=float)
                    if getattr(cfg.processor.inverse_kinematics, "end_effector_position_offset", None)
                    else None
                ),
            ),
            GripperVelocityToJoint(
                ...
                use_gripper_position=(getattr(teleop_device, "name", None) == "so_leader_spacebar_follow"),
            ),
            InverseKinematicsRLStep(
...
def step_env_and_process_transition(
    ...
    raw_observation_override: dict | None = None,
    robot_action_override: torch.Tensor | None = None,
) -> EnvTransition:
    """
    ...
    Args:
        ...
        action: Action to execute (policy/teleop action, e.g. delta)
        env_processor: Environment processor
        action_processor: Action processor
        raw_observation_override: If set (e.g. intervention), use this as joint observation
            so target = FK(obs) + delta uses same moment → follower 跟准 leader.
        robot_action_override: If set (e.g. intervention with joint mirroring), use this for env.step
            instead of pipeline output — 采集模式：直接发主臂关节角给从臂，从臂 action 仍用 pipeline 的给模型学习.
    """

    transition[TransitionKey.ACTION] = action
    if raw_observation_override is not None:
        transition[TransitionKey.OBSERVATION] = raw_observation_override
    else:
    ...
    # 介入且提供 override 时：与采集一致，用主臂关节角直接驱动从臂；给模型学习的 action 仍是 pipeline 结果（delta）
    action_to_robot = (
        robot_action_override
        if robot_action_override is not None
        else processed_action
    )
    obs, reward, terminated, truncated, info = env.step(action_to_robot)
