...
class EEReferenceAndDelta(RobotActionProcessorStep):
    ...
    def action(self, action: RobotAction) -> RobotAction:
        ...
        # 介入时必须用当前观测关节做参考，否则会用上一步的 IK_solution，从臂会“自己动”不跟主臂
        action_from_teleop = comp.get("action_from_teleop", False)
        if action_from_teleop:
            q_raw = None  # 强制走 observation 分支
        elif self.use_ik_solution and "IK_solution" in comp:
            q_raw = comp["IK_solution"]
        else:
            q_raw = None
        if q_raw is None:
            q_raw = np.array(
        ...
        if enabled:
            ref = t_curr
            # 介入时不用锁存参考，每步都以当前观测为参考，从臂严格跟主臂
            if not action_from_teleop and self.use_latched_reference:
...
class EEBoundsAndSafety(RobotActionProcessorStep):
    ...
    Attributes:
        ...
        position_offset: Optional [dx, dy, dz] in meters; added to position before clipping (e.g. [0,0,-0.02] to command lower).
    ...
    def action(self, action: RobotAction) -> RobotAction:
        ...
        if self.position_offset is not None:
            pos = pos + self.position_offset

        # Clip position
        pos = np.clip(pos, self.end_effector_bounds["min"], self.end_effector_bounds["max"])

        # Clip position step to max_ee_step_m to avoid large jumps
...
class GripperVelocityToJoint(RobotActionProcessorStep):
    ...
    where integer actions map to open, close, or no-op. When use_gripper_position is True and
    the action is from teleop (e.g. leader arm), the input is treated as target position 0-1.
    ...
    Attributes:
        ...
        use_gripper_position: If True and action is from teleop, treat gripper as target position in [0, 1].
    """
    ...
    use_gripper_position: bool = False
    ...
    def action(self, action: RobotAction) -> RobotAction:
        ...
        # When action is from teleop (e.g. leader) and use_gripper_position, treat as target position 0–1
        action_from_teleop = self.transition.get(TransitionKey.COMPLEMENTARY_DATA, {}).get(
            "action_from_teleop", False
        )
        if self.use_gripper_position and action_from_teleop:
            gripper_pos = float(np.clip(gripper_vel * self.clip_max, self.clip_min, self.clip_max))
