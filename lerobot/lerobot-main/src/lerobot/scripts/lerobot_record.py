...
def record_loop(
    ...
    while timestamp < control_time_s:
        ...
        # Debug: log policy action periodically to verify robot is being commanded (e.g. if "真机不动")
        if policy is not None and (frame_idx <= 3 or frame_idx % 90 == 1):
            action_preview = dict(list(_sent_action.items())[:4])
            logging.info(
                "Policy→robot action (frame %s): %s ...",
                frame_idx,
                action_preview,
            )
